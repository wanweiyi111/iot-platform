...
public class RedisLock {
     ...
     */
    private int expireMsecs = 60 * 1000;   //这里的值都是默认毫秒单位
     ...
     */
    public synchronized boolean lock() throws InterruptedException { //并发的进入的线程过多且持续，那么这里会产生问题吧，会不会有很多的线程在这里等待且越来越多
        int timeout = timeoutMsecs; //初始化时1000*10
        while (timeout >= 0) {
            long expires = System.currentTimeMillis() + expireMsecs + 1;
            String expiresStr = String.valueOf(expires); // 锁到期时间
            if (this.setNX(lockKey, expiresStr)) { //成功拿到了锁
                // lock acquired
                locked = true;
                return true;
            }
            ...
            if (currentValueStr != null && Long.parseLong(currentValueStr) < System.currentTimeMillis()) { //锁已经失效了
            ...
            timeout -= DEFAULT_ACQUIRY_RESOLUTION_MILLIS; //默认是100毫秒  ,  如果你定义了一个锁并初始化此参=
