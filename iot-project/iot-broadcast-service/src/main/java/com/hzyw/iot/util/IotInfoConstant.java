...
public class IotInfoConstant {
   ...
   // public static final Map<String, Map<String, List<Map<String,String>>>> tomda_charger_relation_tomda_chargersnToNodelist = new HashMap<String, Map<String, List<Map<String,String>>>>();//tomda_charger和node关联关系
    ...
    public static final String dev_dataaccess_value = "Audio";      //每种接入，初始化数据的时候应该要指定接入类型
     ...
     * deviceId
     */ 
    public static final String dev_tomda_charger_id ="Status";  //deviceId
    public static final String dev_tomda_charger_serialNum ="WorkStatus"; //充电桩序列号
    public static final String dev_tomda_charger_station_addr ="Vol"; //站点位置
    ...
    public static void initData(ObjectService objectService){
    	...
    	try{
    		...
    		json.put("vendor_name", "Audio");//汤姆达
    		json.put("type", "Audio"); //充电桩
        	...
        	for(int p=0; p < tomda_charger_data_map.size(); p++){
            	...
            	tomda_charger_iotInfo_.get(serialNum+"_defAttribute").put(dev_tomda_charger_id, 0);   //deviceId
            	tomda_charger_iotInfo_.get(serialNum+"_defAttribute").put(dev_tomda_charger_serialNum, 0); 
            	tomda_charger_iotInfo_.get(serialNum+"_defAttribute").put(dev_tomda_charger_station_addr, 10);   
